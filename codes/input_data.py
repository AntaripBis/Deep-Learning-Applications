...
class SpeechDataGenerator(keras.utils.Sequence):
	...
	def __getitem__(self, index):
		...
		X_IDs_temp = [self.list_IDs[k] for k in indexes]
		Y_IDs_temp = [self.labels[k] for k in indexes]
	...
	def __data_generation(self, X_IDs_temp,Y_IDs_temp):
		...
		# Initialization
		#X = np.empty((self.batch_size,self.n_mfcc,self.audio_dim))
		X = np.empty((self.batch_size,self.audio_dim,self.n_mfcc))
		y = np.empty((self.batch_size,self.sequence_len,self.vocab_size), dtype='int') if self.model_type == "compressed" else np.empty((self.batch_size,1,self.sequence_len*self.vocab_size), dtype='int')
		...
		for i, ID in enumerate(X_IDs_temp):
			...
			# Store class
			if self.model_type == "compressed":
				y[i,] = model_utils.convert_text_to_sequences(Y_IDs_temp[i],self.word_index,self.vocab_size,self.sequence_len)
			else:
				y[i,0,] = model_utils.convert_text_to_longsequence(Y_IDs_temp[i],self.word_index,self.vocab_size,self.sequence_len)
			#y.append(self.labels[i])
...
class SpeechWordDataGenerator(keras.utils.Sequence):
	'Generates data for Keras'
	def __init__(self, list_IDs, labels,vocab_size,batch_size=20,n_mfcc=20,audio_dim=25,shuffle=True):
		'Initialization'
		self.batch_size = batch_size
		self.labels = labels
		self.list_IDs = list_IDs
	...
	def __data_generation(self, list_IDs_temp):
		...
		# Initialization
		X = np.empty((self.batch_size,self.n_mfcc,self.audio_dim))
		#print("Vocab size in data generation : %s" % str(self.vocab_size))
		y = np.empty((self.batch_size,self.vocab_size), dtype='int') 
